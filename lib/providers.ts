...
function sseBlockData(block:string){return block.split(/\r?\n/).filter(l=>l.startsWith('data:')).map(l=>l.slice(5).trimStart()).join('\n');}
export async function* sseData(stream:ReadableStream<Uint8Array>){const reader=stream.getReader();const decoder=new TextDecoder();let buffer='';try{while(true){const {done,value}=await reader.read();buffer+=decoder.decode(value,{stream:!done});let m;while((m=/\r?\n\r?\n/.exec(buffer))){const block=buffer.slice(0,m.index);buffer=buffer.slice(m.index+m[0].length);const data=sseBlockData(block);if(data)yield data;}if(done){const tail=sseBlockData(buffer);if(tail)yield tail;break;}}}finally{await reader.cancel().catch(()=>{});reader.releaseLock();}}
export async function* generate(provider:string,key:string,model:string,system:string,messages:ChatTurn[],signal:AbortSignal,options:{maxTokens?:number;reasoning?:string;onMeta?:(value:Record<string,unknown>)=>void}={}){
 ...
 let response:Response;try{response=await fetch(url,{method:'POST',headers,body:JSON.stringify(payload),signal,redirect:'error'});}catch(e){if(signal.aborted)throw e;throw new ProviderFailure(0,0,'The provider connection failed or timed out.','transport_error');}
