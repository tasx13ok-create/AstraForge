...
export function providerHint(status:number,code:string,provider=''){
 ...
 if(status===403)return 'The API account is not permitted to use this service or feature. Check model entitlement, project permissions and plan restrictions.';
 ...
 if(status===429)return 'The service reported a rate or quota limit. Check the error code and API billing; wait for the reported reset before retrying a temporary rate limit.';
 if(status>=500)return 'The upstream service is temporarily unavailable. Try again later.';
 if(provider==='browserbase')return 'Check the Browserbase project ID and session settings. Persistent sessions require a plan with keep-alive support.';
