...
import {getBrowserView,navigateBrowser,publicUrl,readBrowser,startBrowser,stopBrowser} from '@/lib/remote-browser';
export async function GET(req:Request){try{const owner=await actor(req),id=new URL(req.url).searchParams.get('project');await projectFor(owner,id);try{return Response.json({session:await getBrowserView(owner,id!)});}catch{return Response.json({session:null});}}catch(e){return fail(e);}}
export async function POST(req:Request){try{const owner=await actor(req),b=await body(req),p=await projectFor(owner,b.project);
 ...
 catch(e){await db().prepare("UPDATE approvals SET state='unknown' WHERE id=? AND owner=?").bind(b.approval,owner).run();const detail=e instanceof Error?e.message:'No confirmed result.';await record(owner,p.id,'browser.failed',{operation:JSON.parse(approval.payload).operation,error:detail});throw new Error(detail+' Check the Browserbase session list before retrying an uncertain start.');}
 }
