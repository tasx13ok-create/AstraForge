...
export async function POST(req:Request){try{const owner=await actor(req),b=await body(req),p=await projectFor(owner,b.project);
 if(b.action==='start'){
 const goal=String(b.goal||'').trim();if(!goal||goal.length>15000)throw new Error('Enter a goal under 15,000 characters.');const routes=(await db().prepare('SELECT provider,model FROM connections WHERE owner=? AND enabled=1 ORDER BY created').bind(owner).all<{provider:string;model:string}>()).results.filter(c=>['openai','anthropic','google'].includes(providerCatalog[c.provider]?.kind));if(!routes.length)throw new Error('Connect an AI engine before starting an agent.');
 const preferred=routes.find(r=>r.provider===b.provider)||routes[0];const model=typeof b.model==='string'&&b.model.length<=180&&b.provider===preferred.provider?b.model:preferred.model;const config={provider:preferred.provider,model,reasoning:['auto','low','medium','high'].includes(b.reasoning)?b.reasoning:'auto',maxTokens:Math.min(16384,Math.max(1024,Number(b.maxTokens)||8192)),maxSteps:Math.min(20,Math.max(1,Number(b.maxSteps)||12))};const id=uid();
